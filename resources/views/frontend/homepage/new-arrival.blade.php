...
->selectRaw('products.id,title,selling_price,discount,discount_type,brand_id, slug, feature_image')->whereRaw('id IN (select MAX(id) FROM products GROUP BY category_id)')->orderBy('id', 'desc')->take($section->item_number)->get(); 
?>
@if(count($products)>0)
