...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function adminPasswordRecover(){
        return view('admin.passwords.reset-form');
    }
    public function adminPasswordRecoverNotify(Request $request){
        ...
        $fieldType = filter_var($request->emailOrMobile, FILTER_VALIDATE_EMAIL) ? 'email' : 'mobile';
        $user = Admin::where($fieldType,$emailOrMobile)->first();

        if(!$user){
            return redirect('admin/password/recover')->withErrors(['emailOrMobile' => trans('admin does not exist')]);
        }
        if($fieldType == 'email'){

            $user->email_verification_token = Str::random(32);
            $user->mobile_verification_token = 'admin';
            $user->updated_at = Carbon::now()->addHours(1);
            $user->save();
            ...
            if (count(Mail::failures()) > 0) {
                return redirect('admin/password/recover')->withErrors(['error' => trans('A Network Error occurred. Please try again.')]);
            } else {

                return redirect('admin/password/recover')->with('status', trans('Password reset link has been sent to your email address.'));
            }
        }else{
            $this->validate($request, [
                'emailOrMobile' => 'min:11|numeric|regex:/(01)[0-9]/',
            ...
            if ($user) {
                $msg = Config::get('siteSetting.site_name'). ' Password reset verification (OTP) code: '.$code;
                $response = $this->sendSms($user->mobile,$msg);
                $url = 'admin/password/recover?mobile='.$user->mobile;
                return redirect($url)->with('status', trans('Password reset verification code has been sent to your mobile number.'));
            } else {
                return redirect('admin/password/recover')->withErrors(['error' => trans('A Network Error occurred. Please try again.')]);
            }
    ...
    public function adminPasswordRecoverVerify(Request $request){
        $user = null;
        ...
        if($request->email && $request->token){
            ...
            if($user){
              ...
              return redirect('admin/password/recover')->withErrors(['emailOrMobile' => trans('Invalid email address or token.')]);
            }
        }elseif($request->mobile && $request->otp_code){
            $this->validate($request, [
                'mobile' => 'min:11|numeric|regex:/(01)[0-9]/',
            ]);
            $user = Admin::where('status', 'active');
            ...
            return redirect('admin/password/recover')->with('error', 'Sorry Error occurred. Please try again.');
        }
    }

    public function adminPasswordRecoverUpdate(Request $request)
    {
        //Validate input
        $request->validate([
            'password' => 'required|min:6|confirmed'
