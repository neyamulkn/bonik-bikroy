...
class ReportController extends Controller
{
    ...
    public function reportForm(Request $request){
        ...
        $data['reportReasons'] = ReportReason::where('type', $request->type)->whereNull('product_id')->where('status', 1)->get();
    ...
    public function sellerReport(Request $request){
        ...
        if($request->vendor_id){
            $report->vendor_id = ($request->vendor_id) ? $request->vendor_id : null;
            $report->type = 'seller';
        }
