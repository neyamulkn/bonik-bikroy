...
class PackagePurchaseController extends Controller
{
    ...
    public function packagePurchasePayment(Request $request, $id){
        ...
        elseif($request->payment_method == 'sslcommerz'){
            ...
            //redirect ShurjopayPaymentController for payment process
            $shurjopay = new ShurjopayPaymentController();
            return $shurjopay->shurjopayPayment();
        }elseif($request->payment_method == 'paycorp'){
            //redirect PaycorpController for payment process
            $paycorp = new PaycorpController();
            return $paycorp->paycorpPayment();
    ...
    public function paymentSuccess(){
        ...
        if($payment_data && $payment_data['status'] == 'success') {
            $purchasePackages = PackagePurchase::where('order_id', $payment_data['order_id'])->get();
            if (count($purchasePackages)>0) {
