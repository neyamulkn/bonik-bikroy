...
class ProductController extends Controller
{
    ...
    public function index(Request $request, $statusType='')
    {

        if($statusType == 'reject' || $statusType == 'active' || $statusType == 'deactive'){
            $check_status = 'all';
        }else{
        ...
        if($statusType && $statusType != 'trash' && $statusType != 'promoted' && $statusType != 'all'){
            $products->where('status', $statusType);
        }
        //deleted post
        if($statusType == 'trash'){
        ...
        //promoted post
        if($statusType == 'promoted'){
            $promotePosts =  PromoteAds::where('start_date', '<=', now())->where('end_date', '>=', now())->where('status', 1)->pluck('ads_id')->toArray();
